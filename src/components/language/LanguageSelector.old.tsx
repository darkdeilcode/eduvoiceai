...
import { Languages, Clock, Zap, AlertCircle, Video, Brain, MessageSquare, Users, Globe, Coins, Key } from "lucide-react";
...
export function LanguageSelector({ onStartTest, onStartVideoTest, userTokenBalance, isLoading = false }: LanguageSelectorProps) {
  ...
  const handleStartTest = () => {
    ...
    const config: LanguageTestConfig = {
      ...
      difficulty,
      testType: testMode === 'video-conference' ? 'video-conference' : testType,
      duration: testMode === 'video-conference' || testMode === 'conversational' ? 10 : duration // Fixed 10 minutes for video/conversational modes
    };
    ...
    const tavusConfig: TavusAPIConfig | undefined = enableTavusVideos ? {
      ...
      personaId: testMode === 'conversational' && cviPersonaId ? cviPersonaId : undefined
    } : undefined;

    const cviOptions = testMode === 'conversational' ? {
      replicaId: cviReplicaId || undefined,
      personaId: cviPersonaId || undefined
    } : undefined;

    if ((testMode === 'video-conference' || testMode === 'conversational') && onStartVideoTest) {
      onStartVideoTest(config, tavusConfig, testMode === 'conversational', cviOptions);
    } else {
  ...
  const getTestTypeDescription = (type: string) => {
    switch (type) {
      case "conversation":
        return "Interactive dialogue with AI avatar";
      case "pronunciation":
        return "Focus on accent and pronunciation accuracy";
      case "comprehension":
        return "Listening and reading comprehension tests";
      case "mixed":
        return "Comprehensive test covering all skills";
      default:
        return "";
    }
  };
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Select between standard language test or video conference speaking test
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Tabs value={testMode} onValueChange={(value: any) => setTestMode(value)} className="w-full">
            <TabsList className="grid w-full grid-cols-3">
              <TabsTrigger value="standard" className="flex items-center gap-2">
                <Brain className="h-4 w-4" />
                Standard Test
              </TabsTrigger>
              <TabsTrigger value="video-conference" className="flex items-center gap-2">
            ...
            <TabsContent value="standard" className="mt-4">
              <div className="space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-medium">Standard Language Test</h3>
                </div>
                <p className="text-sm text-muted-foreground">
                  Comprehensive language assessment with multiple question types, customizable duration, and AI-powered evaluation.
                </p>
                <div className="flex flex-wrap gap-2">
                  <Badge variant="outline">Multiple Skills</Badge>
                  <Badge variant="outline">Flexible Duration</Badge>
                  <Badge variant="outline">Detailed Reports</Badge>
                </div>
              </div>
            </TabsContent>
            
            <TabsContent value="video-conference" className="mt-4">
              <div className="space-y-3">
                <div className="flex items-center gap-2">
                  <Users className="h-5 w-5 text-green-600" />
                  <h3 className="font-medium">Video Conference Speaking Test</h3>
                </div>
                <p className="text-sm text-muted-foreground">
                  IELTS-style speaking test with AI examiner via video call. 10-minute structured interview with real-time interaction.
                </p>
                <div className="flex flex-wrap gap-2">
                  <Badge variant="outline">IELTS Format</Badge>
                  <Badge variant="outline">10 Minutes</Badge>
                  <Badge variant="outline">Video Call</Badge>
                  <Badge variant="outline">AI Examiner</Badge>
                </div>
                ...
                <Alert>
                  ...
                  <AlertDescription>
                    Camera and microphone access required. Test follows IELTS speaking format with 3 parts.
                  </AlertDescription>
                </Alert>
              </div>
            </TabsContent>
            ...
            <TabsContent value="conversational" className="mt-4">
              <div className="space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-medium">AI Conversational Video Interface</h3>
                </div>
                <p className="text-sm text-muted-foreground">
                  Natural conversation with Tavus AI avatar in your target language. 10-minute interactive session with real-time feedback.
                </p>
                <div className="flex flex-wrap gap-2">
                ...
                <Alert>
                  <AlertCircle className="h-4 w-4" />
                  <AlertTitle>Powered by Tavus</AlertTitle>
                  <AlertDescription>
                    Uses Tavus Conversational Video Interface for natural AI conversations. Camera and microphone required.
                  </AlertDescription>
                </Alert>
              </div>
            </TabsContent>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <CardHeader>
          ...
          <CardDescription>
            Select your target language and test preferences
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          {/* Duration - only show for standard test */}
          {testMode === 'standard' && (
            <div className="space-y-2">
          ...
          {/* Fixed duration notice for video conference */}
          {testMode === 'video-conference' && (
            <div className="space-y-2">
              <Label>Test Duration</Label>
              <div className="p-3 bg-muted rounded-lg">
                <p className="text-sm font-medium">Fixed: 10 minutes</p>
                <p className="text-xs text-muted-foreground">
                  IELTS speaking test format with structured timing
                </p>
              </div>
            </div>
          )}
          
          {/* Fixed duration notice for conversational mode */}
          ...
          {/* CVI Configuration for conversational mode */}
          {testMode === 'conversational' && (
          ...
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                ...
                <p className="text-sm text-muted-foreground">
                  Use Tavus.io video avatars for test questions
                </p>
              </div>
              <Switch 
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <p className="text-muted-foreground capitalize">
                  {testMode === 'video-conference' ? 'Video Conference' : testType.replace("_", " ")}
                </p>
              </div>
              <div>
                <p className="font-medium">Duration</p>
                <p className="text-muted-foreground">
                  {testMode === 'video-conference' ? '10' : duration} minutes
                </p>
              </div>
            </div>
